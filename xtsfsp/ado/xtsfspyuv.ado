...
program Replay
	...
	global diparmopt diparm(Wy, label(rho) prob function($rymin/(1+exp(@))+$rymax*exp(@)/(1+exp(@)))  d(exp(@)*(($rymax-$rymin)/(1+exp(@))^2))) diparm(Wv, label(gamma) prob function($rvmin/(1+exp(@))+$rvmax*exp(@)/(1+exp(@)))  d(exp(@)*(($rvmax-$rvmin)/(1+exp(@))^2))) diparm(Wu, label(tau) prob function($rumin/(1+exp(@))+$rumax*exp(@)/(1+exp(@)))  d(exp(@)*(($rumax-$rumin)/(1+exp(@))^2)))
